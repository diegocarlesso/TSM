'use strict';
/** Ferramentas: biblioteca de comandos, túneis, gravação de sessão e chaves SSH. */
import {
 ...
 * `run: true` acrescenta Enter; `false` só digita, para o usuário revisar.
 */
    ...
    if (!items.length) {
      list.append(el('p', {
        ...
        text: all.length
          ...
          : 'Nenhum comando guardado ainda. Guarde os que você repete todo dia.'
      }));
    }
...
async function editSnippet(snippet) {
  ...
  const ok = await modal({
    ...
    render: () => el('div', { class: 'form-grid' }, [
      ...
      el('input', {
        type: 'text', value: model.category, placeholder: 'Diagnóstico, Deploy, Rede…',
      ...
      el('div', { class: 'hint', text: 'Desmarcado, o comando só é digitado — você revisa antes de rodar.' })
    ]),
    footer: (api) => [
  ...
  if (!ok) return;
  if (!model.name.trim() || !model.content.trim()) return toast('Nome e comando são obrigatórios', 'err');
...
export async function tunnelsDialog(pane) {
  ...
  async function rerender() {
    ...
    for (const f of forwards) {
      tbody.append(el('tr', {}, [
        ...
        el('td', {}, el('button', {
          class: 'icon-btn', text: '✕', title: 'Fechar túnel',
          onClick: async () => {
    ...
    body.replaceChildren(
      el('p', { class: 'muted', text: `Sessão: ${target.name} (${target.target || ''})` }),
      forwards.length ? table : el('p', { class: 'muted', text: 'Nenhum túnel aberto nesta sessão.' }),
      el('div', { style: 'margin-top:12px' }, [
        el('button', { text: '+ Abrir túnel', onClick: () => addTunnel(target).then(rerender) })
      ])
    );
  }
  ...
  await rerender();
  await modal({
    title: 'Túneis (encaminhamento de portas)',
    width: 700,
    render: () => body,
    footer: (api) => [el('button', { class: 'primary', text: 'Fechar', onClick: () => api.close(true) })]
  });
    ...
    render: () => {
      ...
      const refresh = () => {
        hint.textContent = model.type === 'local'
          ? 'Abre uma porta NESTA máquina que sai pelo servidor (equivale a ssh -L).'
          : 'Abre uma porta NO SERVIDOR que volta para esta máquina (equivale a ssh -R).';
      };
      refresh();
...
export async function sessionLogDialog(pane) {
  ...
  if (status.active) {
    const parar = await confirmDialog({
      ...
      detail: `${status.filePath}\n\n${formatBytes(status.bytes)} gravados até agora.`,
      confirmLabel: 'Parar gravação',
      danger: true
    });
    if (parar) {
  ...
  const ok = await modal({
    title: `Gravar sessão — ${target.name}`,
    width: 580,
    render: () => el('div', { class: 'form-grid' }, [
      ...
      el('div', { class: 'full' }, [
        checkbox('Remover códigos de cor ANSI (log legível em editor)',
      ...
      el('div', { class: 'full' }, [
        checkbox('Acrescentar ao arquivo se já existir', model.append, (v) => { model.append = v; })
      ])
    ]),
    footer: (api) => [
      el('button', { text: 'Cancelar', onClick: () => api.close(false) }),
      el('button', { class: 'primary', text: 'Começar a gravar', onClick: () => api.close(true) })
    ]
  });
...
export async function keysDialog() {
  ...
  async function rerender() {
    ...
    table.append(el('thead', {}, el('tr', {}, [
      ...
      el('th', { text: 'Impressão digital' }), el('th', { text: '' })
    ])));
    const tbody = el('tbody');
    for (const k of keys) {
      tbody.append(el('tr', { title: k.path }, [
        ...
        el('td', {}, el('div', { class: 'inline' }, [
          el('button', {
            class: 'icon-btn', text: '⧉', title: 'Copiar chave pública',
            ...
            onClick: async () => {
              ...
              if (pub) {
                ...
                toast('Chave pública copiada', 'ok');
              } else {
                toast('Não foi possível ler a chave pública', 'warn');
              }
            }
          }),
          el('button', {
...
async function generateKey() {
  ...
  const ok = await modal({
    ...
    render: () => {
      ...
      const grid = el('div', { class: 'form-grid' }, [
        ...
        el('div', {
          class: 'hint',
          text: 'Sem senha a chave funciona sozinha — prático para automação, ' +
  ...
  if (!ok) return;
  if (model.passphrase !== model.confirm) return toast('As senhas não conferem', 'err');
  ...
  await modal({
    ...
    render: () => el('div', {}, [
      ...
      el('p', { class: 'muted', text: 'Copie a chave pública abaixo para o ~/.ssh/authorized_keys do servidor:' }),
      el('textarea', {
